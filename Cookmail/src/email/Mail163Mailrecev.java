 ...
 *
 * @author Cryin'
 * update at 2013-05-01
 * 
 */
public class Mail163Mailrecev extends Mail {
    ...
    public Mail163Mailrecev( MailFuncInter mailFuncInter )
    {
	...
	try {
		...
		if (strHtml.indexOf("errorType=Login_Timeout") != -1) 
                {
                    m_mailFuncInter163.ShowMessage(m_accounts + " :Cookie已失效!");
                    return;
                }
                
                if (urls.indexOf("mail.163.com") != -1) 
                {
	...
	int index;
        m_mailFuncInter163.ShowMessage(m_accounts+" :开始登录");
	url = "https://ssl.mail.163.com/entry/coremail/fcg/ntesdoor2?df=mail163_letter&from=web&funcid=loginone&iframe=1&language=-1&net=t&passtype=1&product=mail163&race=78_78_187_gz&style=-1&uid="+m_accounts+"@163.com";
	NameValuePair[] InData = { 
			new NameValuePair("savelogin", "1"),
			new NameValuePair("url2", "http://mail.163.com"),
    ...
    private void getMailAddress(String strHtml)
    {
                ...
                try {
                            ...
                            url="http://mail.163.com/contacts/call.do?from=webmail";
                            ...
                            m_mailFuncInter163.ShowMessage(m_accounts+" :开始下载通讯录!");
                            ...
                            m_mailFuncInter163.ShowMessage(m_accounts+" :通讯录下载完成!");
                            //下载通讯录代码结尾
        } catch (Exception ex) {
			m_mailFuncInter163.ShowMessage(m_accounts+" :下载通讯录失败！"+ ex.getMessage());
	}
    }
    ...
    private void getBoxName(String strHtml) // 取箱子
	{
		...
		try {
			...
			if (urls.indexOf("mail.163.com") != -1)
				...
				m_mailFuncInter163.ShowMessage(m_accounts+" :取箱子失败！");
			}
		} catch (Exception ex) {
			m_mailFuncInter163.ShowMessage(m_accounts+" :取箱子失败！"+ ex.getMessage());
		}
	}
		...
		try {
			...
			while ((mailIndex = strindata.indexOf("<object>", mailIndex)) != -1) {
				...
				if (mailId != "-1" && mailId != "") {
					...
					{
						m_mailFuncInter163.ShowMessage(m_accounts+" :下载失败:"+ex.getMessage());
					}
						// / 根据时间判断 是否收取
					...
					/*	if (ifRecv) {
							...
							try {
								...
								try {
									...
									sql = "insert into 163mailId(Name,MsgId,DownTime,MailType) values('"
											+ m_username
											+ "','"
											+ mailId
											+ "','"
			...
			m_mailFuncInter163.ShowMessage(m_accounts+" :取邮件失败!" + ex.getMessage());
		}
    }
    /**
    * Get
    ...
    */
    private String getRequest(String url) throws HttpException, IOException
    {
		...
		try
		{
			...
			{
				m_mailFuncInter163.ShowMessage(m_accounts+" :163 Request Method failed: "
