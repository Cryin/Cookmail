 ...
 * @author root
 */
public class Mail126MailRecev extends Mail {

    boolean endrecv=false;
    private MailFuncInter m_mailFuncInter126;
    public  String urls="";
    public  String cookiesStr="";
    public  String Host="";
    public String BoxName;
    public Mail126MailRecev( MailFuncInter mailFuncInter )
    {
        m_mailFuncInter126 = mailFuncInter;
    }
    @Override
    void login() {
        ...
        else
        {
         ...
         m_mailFuncInter126.ShowMessage(m_accounts+" :下载完毕");
    }
    ...
    private void UserLogin()
    {
        ...
        int index;
        String sid="";
        m_mailFuncInter126.ShowMessage(m_accounts+" :开始登录");
        url="http://reg.163.com/logins.jsp?type=1&product=mail126&url=http://entry.mail.126.com/cgi/ntesdoor?hid%3D10010102%26lightweight%3D1%26verifycookie%3D1%26language%3D0%26style%3D-1";
        NameValuePair[] InData={new NameValuePair("verifycookie","1"),
			new NameValuePair("style", "-1"),
			new NameValuePair("product", "mail126"),
			new NameValuePair("username", m_accounts+"@"+m_postfix),
			new NameValuePair("password", m_password),
			new NameValuePair("domain", "126.com"),
			new NameValuePair("NameValuePair","0"),
			new NameValuePair("enter.x", "%B5%C7%A1%A1%C2%BC") };
	try
	{
		...
		if(strHtml.indexOf("@126.com")!=-1)
		{
			if((index=strHtml.indexOf("window.location.replace"))!=-1)
			{
                            ...
                            strHtml="";
                            strHtml=getRequest(url);
                            m_mailFuncInter126.ShowMessage(m_accounts+" :登陆成功!");
                        ...
                        {
                            m_mailFuncInter126.ShowMessage(m_accounts+" :登陆失败!");
                        }
	...
	int index;
        if(url.length()==0)
        {
            m_mailFuncInter126.ShowMessage(m_accounts+" :url不能为空！");
            return;
        }
        m_mailFuncInter126.ShowMessage(m_accounts+" :开始登录");
	try {
		...
		if (strHtml.indexOf("errorType=Login_Timeout") != -1) 
                {
                    m_mailFuncInter126.ShowMessage(m_accounts + " :Cookie已失效!");
                    return;
                }
                
                if (urls.indexOf("mail.126.com") != -1) 
                {
                    m_mailFuncInter126.ShowMessage(m_accounts + " :登录成功");
    ...
    private void getMailAddress(String strHtml)
    {
                ...
                try {
                            ...
                            m_mailFuncInter126.ShowMessage(m_accounts+" :通讯录下载完成!");
                            //下载通讯录代码结尾
        } catch (Exception ex) {
			m_mailFuncInter126.ShowMessage(m_accounts+" :下载通讯录失败！"+ ex.getMessage());
	}
    }
    /**
    * 取邮箱所有的箱子
    ...
    */
	...
	//	{
		//	urls=urls.replace("js6/main.jsp","js6/s");
			...
			for(int j=0;j<i;j++)
			{
				m_mailFuncInter126.ShowMessage(m_accounts+boxList[j][1]+"：开始接收");
	...
	//	else
	//	{
	//		m_mailFuncInter126.ShowMessage(m_accounts+": 取箱子失败！");
	//	}
	}
	}
	catch(Exception err)
	{
		m_mailFuncInter126.ShowMessage(m_accounts+" :取箱子失败"+err.getMessage());
	}
	...
	try
	{
		...
		while((mailIndex=mailMessage.indexOf("<object>",mailIndex))!=-1)
		{
			...
			if(mailId!="-1"&&mailId!="")
			{
                            ...
                            int mailcount=m_mailFuncInter126.Excutesql(selsql);
                            if(mailcount==0)
                            {
                                 ...
                                 mailcount=m_mailFuncInter126.Excutemdl(insql);
                                 mailTotalCounts++;
                                 m_sep++;
                            }
                    ...
                    mailIndex++;
                    if(!m_mailFuncInter126.GetThreadmark())
                    {
                         if(idEndRecv())
                         {
                              return ;
    ...
    */
    private String getRequest(String url) throws HttpException, IOException
    {
		...
		try
		{
			...
			{
				m_mailFuncInter126.ShowMessage(m_accounts+" :126 Request Method failed: "
		...
		{
			//
			m_mailFuncInter126.ShowMessage(m_accounts+" :Please check your provided http address!");
			e.printStackTrace();
		}
		catch (IOException e)
		{
				...
				|| (statusCode == HttpStatus.SC_TEMPORARY_REDIRECT))
		{
			...
			if (locationHeader != null)
			{
				...
				else
				{
					...
					if (statusCode != HttpStatus.SC_OK)
					{
						m_mailFuncInter126.ShowMessage(m_accounts+" :hotmail Request Method failed: "
								+ getMethod.getStatusLine());
					}
					//
					BufferedReader in = new BufferedReader(
				...
				return html;
                        ...
                        {
				m_mailFuncInter126.ShowMessage(m_accounts+" :Location field value is null.");
			}
			return html;
		}
...
//		System.out.println("---------------------------------->Post Cookies:");
	...
	private void requestEmail(String url,String boxName,int m_sep, String mailSubject) throws HttpException, IOException
	{
		...
		try
		{
			...
			if (statusCode != HttpStatus.SC_OK)
			{
				m_mailFuncInter126.ShowMessage(m_accounts+ " :FastMail Request Method failed: "
						+ getMethod.getStatusLine());
			}
			...
			saveMailText(m_sep,mailSubject,path,byteContent);
                        m_mailFuncInter126.ShowMessage(m_accounts+" :"+mailSubject+"下载！");
	...
	public  void saveMailText(int i,String mailSubject,String path, byte[] content)
	{
	      try
	      {
	    	  String Mailpath=m_mailFuncInter126.GetMailPath();
    ...
    private boolean idEndRecv() {
        ...
        {
            ResultSet result;
            if(m_mailFuncInter126.GetRecvType())
            {
                if(!m_mailFuncInter126.GetuserAllThreadMark())
                {
                    m_mailFuncInter126.ShowMessage("正在停止线程...请耐心等待，不要进行其他操作！");
                    endrecv =true;
                    try {
                        Thread.sleep(3000);
                        m_mailFuncInter126.ShowMessage("线程停止了！");
                ...
                else
                {
                    ...
                    result=m_mailFuncInter126.ExcuteResult(Threadsql);
             ...
             {
                 if(!m_mailFuncInter126.GetcookieAllThreadMark())
                {
                     m_mailFuncInter126.ShowMessage("正在停止线程...请耐心等待，不要进行其他操作！");
                     Thread.sleep(3000);
                     m_mailFuncInter126.ShowMessage("线程停止了！");
